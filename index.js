...
[Response Format]:
Return ONLY the modified full text. Do NOT wrap in markdown blocks.
`;
    } else {
    ...
    <div class="pw-wrapper">
        ...
        <div id="pw-view-editor" class="pw-view active">
            <div class="pw-scroll-area">
                ...
                <div id="pw-result-area" style="display:none; margin-top:15px;">
                    <div class="pw-relative-container">
                        ...
                        <i class="fa-solid fa-expand pw-expand-btn" id="pw-btn-expand" title="全屏编辑"></i>
                    </div>
            ...
            <div class="pw-footer">
                ...
                <div class="pw-footer-right">
                    <label class="pw-wi-check-container" title="将设定同步保存到世界书条目"><input type="checkbox" id="pw-wi-toggle" checked><span>同步进世界书</span></label>
        ...
        <div id="pw-diff-overlay" class="pw-diff-container" style="display:none;">
            ...
            <div class="pw-diff-body">
                ...
                <div class="pw-diff-col">
                    <div class="pw-diff-label" style="color:#9ece6a">修改后 (可编辑调整)</div>
                    <textarea id="pw-diff-new" class="pw-diff-box editable"></textarea>
                </div>
            </div>
            <div class="pw-diff-actions">
                <button class="pw-btn danger" id="pw-diff-cancel" style="width:auto;">放弃修改</button>
                <button class="pw-btn save" id="pw-diff-confirm" style="width:auto;">接受修改</button>
            </div>
        </div>
        ...
        <div id="pw-fullscreen-overlay" class="pw-fullscreen-overlay">
            ...
            <div class="pw-refine-toolbar" style="margin-top:0;">
                ...
                <div class="pw-refine-actions">
                    <div class="pw-tool-btn" id="pw-fs-insert-selection"><i class="fa-solid fa-quote-left"></i> 引用</div>
                    <div class="pw-tool-btn" id="pw-fs-refine-btn"><i class="fa-solid fa-magic"></i> 润色</div>
                </div>
            </div>
        </div>
...
function bindEvents() {
    ...
    // 引用逻辑 (多选无限追加)
    const handleInsert = (textAreaId, inputId) => {
    ...
    const handleRefine = async (sourceInputId, sourceTextId) => {
        ...
        const currentRawText = $(sourceTextId).val();
        
        const $btn = $('#pw-btn-refine, #pw-fs-refine-btn');
        ...
        try {
            const config = {
                ...
                indepApiKey: $('#pw-api-key').val(), indepApiModel: $('#pw-api-model').val()
            };
            ...
            $input.val(''); adjustHeight($input[0]);
            
        } catch (e) { toastr.error(e.message); }
        finally { $btn.html(oldHtml); }
    };
    ...
    $(document).on('click.pw', '#pw-clear', function() {
        if(confirm("清空？")) {
            ...
            $('#pw-result-text').val(''); $('#pw-refine-input').val('');
            saveCurrentState();
        }
    });
    $(document).on('click.pw', '#pw-snapshot', function() {
        ...
        saveHistory({ request: $('#pw-request').val() || "无请求", timestamp: new Date().toLocaleString(), targetChar: "User", data: { name: "User", resultText: curText } });
    ...
    $(document).on('click.pw', '#pw-btn-apply', async function() {
        ...
        try { await forceSavePersona(name, finalContent); toastr.success(TEXT.TOAST_SAVE_SUCCESS(name)); } 
        catch (e) { toastr.error(e.message); return; }

        if ($('#pw-wi-toggle').is(':checked')) {
            const boundBooks = await getContextWorldBooks();
            let targetBook = window.pwExtraBooks?.[0] || boundBooks[0];
            if (targetBook) {
                try {
                    ...
                    if (r.ok) {
                        ...
                        if (targetId === -1) { const ids = Object.keys(d.entries).map(Number); targetId = ids.length ? Math.max(...ids) + 1 : 0; }
