...
2. If text is quoted, focus on that part.
3. Maintain the list format.
4. User Name: "${currentName}" (Immutable).

[Response Format]:
Return ONLY the modified text content.
`;
    } else {
...
function bindEvents() {
    ...
    $(document).on('click.pw', '#pw-insert-selection', function() {
        ...
        if (textarea) {
            ...
            if (selectedText) {
                ...
                const cur = $input.val();
                $input.val(cur + ` 将 "${selectedText}" 修改为: `).focus();
            } else {
                toastr.info("请先在结果框中划选文字");
            }
        }
    });
    ...
    $(document).on('click.pw', '#pw-wi-refresh', async () => {
        ...
        const options = availableWorldBooks.length > 0 
