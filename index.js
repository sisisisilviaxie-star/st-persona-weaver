...
const defaultSettings = {
    autoSwitchPersona: true, syncToWorldInfo: false, // 默认不勾选
        ...
        <div id="pw-view-editor" class="pw-view active">
            ...
            <div class="pw-footer">
                ...
                <div class="pw-footer-right">
                    <label class="pw-wi-check-container" title="仅同步到角色绑定的世界书"><input type="checkbox" id="pw-wi-toggle"><span>同步进世界书</span></label>
...
function bindEvents() {
    ...
    $(document).on('click.pw', '#pw-btn-refine', async function() {
        ...
        try {
            // [核心] 调用 collectActiveWorldInfoContent 获取真实世界书内容
            const wiContent = await collectActiveWorldInfoContent();

            const config = { 
                mode: 'refine', request: refineReq, currentText: oldText, 
                wiContext: wiContent, // 传入实际抓取到的内容
                ...
                if (!isChanged) {
                    optionsHtml = `
                        <div class="pw-diff-options">
                            <div class="pw-diff-opt single-view selected" data-val="${valNew}">
                                <span class="pw-diff-opt-label">无变更</span>
                                <div class="pw-diff-opt-text">${valNew}</div>
                            </div>
                        </div>`;
                } else {
                    optionsHtml = `
                        <div class="pw-diff-options">
                            <div class="pw-diff-opt old diff-active" data-val="${valOld}">
                                <span class="pw-diff-opt-label">原版本</span>
                                <div class="pw-diff-opt-text">${valOld || "(无)"}</div>
                            </div>
                            <div class="pw-diff-opt new selected diff-active" data-val="${valNew}">
                                <span class="pw-diff-opt-label">新版本</span>
                ...
                const $row = $(`
                    <div class="pw-diff-row" data-key="${key}">
                        ...
                        <div class="pw-diff-edit-area">
                            <textarea class="pw-diff-custom-input" placeholder="可微调...">${valNew}</textarea>
                        </div>
                    </div>
                `);
                $list.append($row);
    ...
    $(document).on('click.pw', '#pw-diff-confirm', function() {
        ...
        $('.pw-diff-row').each(function() {
            ...
            if (!val) return;
            if (key.startsWith('Info_')) finalLines.push(val);
