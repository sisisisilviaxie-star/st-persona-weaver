...
Task: Create or update a User Persona based on the input.
...
[Requirements]:
1. Return ONLY a list of attributes in "Key: Value" format.
2. NO markdown, NO thoughts, NO explanations.
3. User Name: "{{user}}" (Do not change).
4. If modifying, keep existing keys unless instructed otherwise.
...
function parseTextToMap(text) {
    ...
    lines.forEach(line => {
        ...
        if (!cleanLine || 
            ...
            cleanLine.startsWith('---') || 
            cleanLine.toLowerCase().includes('original string') || // 过滤 Diff 标记
            cleanLine.match(/^(\[.*\]|\(.*\))$/)) { // 过滤 [Note] 或 (Note)
            return;
        }

        // 2. 尝试分离 Key: Value
        // 支持中文冒号，支持 Key 带引号
        const idx = cleanLine.indexOf(':') !== -1 ? cleanLine.indexOf(':') : cleanLine.indexOf('：');
        
        if (idx !== -1) {
            let key = cleanLine.substring(0, idx).trim();
            ...
            if (cleanLine.length > 5) {
                map.set(`Info_${Math.random().toString(36).substr(2, 4)}`, cleanLine);
            }
        }
    });
    return map;
...
// 核心生成逻辑 (使用自定义 Prompt)
async function runGeneration(data, apiConfig, customPrompt) {
    const context = getContext();
    const charId = context.characterId;
    const charName = (charId !== undefined) ? context.characters[charId].name : "None";
    const scenario = (charId !== undefined) ? (context.characters[charId].scenario || "") : "";
    ...
    let instruction = "";
    if (data.mode === 'refine') {
        instruction = `Current Data: """${data.currentText}"""\nRefinement Request: "${data.request}"`;
    } else {
        const targetKeys = tagsCache.map(t => t.name).filter(n => n).join(', ');
        instruction = `Generate a new Persona based on: "${data.request}".\nRecommended Keys: ${targetKeys}`;
    }
    ...
    systemPrompt = systemPrompt
        ...
        .replace(/{{wi}}/g, wiText)
        .replace(/{{input}}/g, data.mode === 'refine' ? data.currentText : data.request) // 兼容旧模板
...
async function openCreatorPopup() {
    ...
    if (!extension_settings[extensionName].customPrompt) {
        extension_settings[extensionName].customPrompt = DEFAULT_SYSTEM_PROMPT;
        ...
        <div id="pw-view-editor" class="pw-view active">
            ...
            <div class="pw-footer">
                <div class="pw-footer-left">
                    ...
                    <div class="pw-mini-btn" id="pw-snapshot"><i class="fa-solid fa-save"></i> 历史</div>
                </div>
                <div class="pw-footer-right">
                    <label class="pw-wi-check-compact" title="仅同步到角色绑定的世界书"><input type="checkbox" id="pw-wi-toggle" ${wiChecked ? 'checked' : ''}><span>同步WI</span></label>
                    <button id="pw-btn-apply" class="pw-btn save save-compact">保存并覆盖</button>
                </div>
            </div>
        </div>
        ...
        <div id="pw-view-api" class="pw-view">
            <div class="pw-scroll-area">
                <div class="pw-card-section">
                    <div style="font-weight:bold;margin-bottom:5px;">系统提示词 (Prompt):</div>
                    <textarea id="pw-custom-prompt" class="pw-prompt-editor">${config.customPrompt}</textarea>
                    <div style="font-size:0.8em;opacity:0.6;margin-top:5px;">支持变量: {{char}}, {{user}}, {{scenario}}, {{wi}}, {{instruction}}</div>
                    <button id="pw-api-save-prompt" class="pw-btn primary" style="margin-top:5px;width:auto;">保存提示词</button>
                </div>
                <div class="pw-card-section"><div class="pw-row"><label>API 来源</label><select id="pw-api-source" class="pw-input" style="flex:1;"><option value="main" ${config.apiSource === 'main'?'selected':''}>主 API</option><option value="independent" ${config.apiSource === 'independent'?'selected':''}>独立 API</option></select></div><div id="pw-indep-settings" style="display:${config.apiSource === 'independent' ? 'flex' : 'none'}; flex-direction:column; gap:15px;"><div class="pw-row"><label>URL</label><input type="text" id="pw-api-url" class="pw-input" value="${config.indepApiUrl}" style="flex:1;"></div><div class="pw-row"><label>Key</label><input type="password" id="pw-api-key" class="pw-input" value="${config.indepApiKey}" style="flex:1;"></div><div class="pw-row"><label>Model</label><div style="flex:1; display:flex; gap:5px; width:100%;"><input type="text" id="pw-api-model" class="pw-input" value="${config.indepApiModel}" list="pw-model-list" style="flex:1;"><datalist id="pw-model-list"></datalist><button id="pw-api-fetch" class="pw-btn primary pw-api-fetch-btn" title="获取模型" style="width:auto;"><i class="fa-solid fa-cloud-download-alt"></i></button></div></div></div><div style="text-align:right;"><button id="pw-api-save" class="pw-btn primary" style="width:auto;">保存设置</button></div></div>
            </div>
        </div>
...
function bindEvents() {
    ...
    const saveCurrentState = () => {
        saveState({
            ...
            localConfig: {
                ...
                customPrompt: $('#pw-custom-prompt').val() // 保存 prompt
            }
        });
    };
    ...
    $(document).on('click.pw', '#pw-btn-refine', async function() {
        ...
        try {
            ...
            // 使用当前编辑框里的 Prompt
            const currentPrompt = $('#pw-custom-prompt').val();
            const responseText = await runGeneration(config, config, currentPrompt);
            ...
            allKeys.forEach(key => {
                ...
                if (!isChanged) {
                    ...
                    optionsHtml = `<div class="pw-diff-options"><div class="pw-diff-opt old diff-active" data-val="${valOld}"><span class="pw-diff-opt-label">原版本</span><div class="pw-diff-opt-text">${valOld || "(无)"}</div></div><div class="pw-diff-opt new selected diff-active" data-val="${valNew}"><span class="pw-diff-opt-label">新版本</span><div class="pw-diff-opt-text">${valNew || "(删除)"}</div></div></div>`;
                }
    ...
    $(document).on('click.pw', '#pw-btn-gen', async function() {
        ...
        try {
            ...
            const currentPrompt = $('#pw-custom-prompt').val();
            const text = await runGeneration(config, config, currentPrompt);
            ...
            if (targetBook) {
                try {
                    ...
                    if (r.ok) {
                        ...
                        d.entries[targetId] = { uid: Number(targetId), key: [name, "User"], content: content, comment: entryName, enabled: true, selective: true };
    ...
    $(document).on('click.pw', '#pw-api-save-prompt', () => { 
        extension_settings[extensionName].customPrompt = $('#pw-custom-prompt').val();
        saveSettingsDebounced();
        toastr.success("Prompt 已更新");
    });
    ...
    $(document).on('click.pw', '#pw-api-save', () => { saveData(); toastr.success("设置已保存"); });
